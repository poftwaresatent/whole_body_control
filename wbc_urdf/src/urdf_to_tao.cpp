...
namespace jspace {
  ...
  // otherwise the ROS_DEBUG_STREAM macro bails with
  // `‘jspace::ros::console’ has not been declared'
  using namespace ::ros;
  ...
  copy_fuse_fixed(element * parent, element * original)
  {
	...
	ROS_DEBUG_STREAM ("    after:      " << fused_duplicate->tao_mass_prop);
...
#define ALLOW_NEGATIVE_AXES
	  ...
	  if (urdf::Joint::REVOLUTE == urdf_joint->type) {
	    ROS_DEBUG_STREAM ("jspace::ros::create_tao_tree(): REVOLUTE joint");
	    tao_joint = new taoJointRevolute(foo);
	  }
	  else if (urdf::Joint::CONTINUOUS == urdf_joint->type) {
	    ROS_DEBUG_STREAM ("jspace::ros::create_tao_tree(): CONTINUOUS joint (treated like a revolute joint<)");
	    tao_joint = new taoJointRevolute(foo);
	  }
	  else if (urdf::Joint::PRISMATIC == urdf_joint->type) {
	    ROS_DEBUG_STREAM ("jspace::ros::create_tao_tree(): PRISMATIC joint");
