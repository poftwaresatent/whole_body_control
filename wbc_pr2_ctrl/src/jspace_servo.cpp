...
static opspace::TaskPostureController * create_task_posture_controller(ros::NodeHandle nn, std::string const & prefix);
...
void load_controllers() {
  ...
  if ( ! ctrl) {
    ...
    controller_lib.insert(make_pair("op_task_posture", ctrl));
  }
}


opspace::TaskPostureController * create_task_posture_controller(ros::NodeHandle nn, std::string const & prefix)
{
  double dt_seconds(0.002);
  if ( ! nn.getParam(prefix + "/dt_seconds", dt_seconds)) {
    ROS_WARN ("missing parameter: %s/dt_seconds", prefix.c_str());
  }
  
  std::string end_effector_name("l_wrist_roll_link");
  if ( ! nn.getParam(prefix + "/end_effector", end_effector_name)) {
    ROS_WARN ("missing parameter: %s/end_effector", prefix.c_str());
  }
  taoDNode const * end_effector(jspace_model->getNodeByName(end_effector_name));
  if ( ! end_effector) {
    ROS_ERROR ("invalid %s/end_effector: `%s'", prefix.c_str(), end_effector_name.c_str());
    return 0;
  }
  
  opspace::TaskPostureController * ctrl(0);
  
  try {
    XmlRpc::XmlRpcValue value;
    
    std::vector<double> control_point;
    if ( ! nn.getParam(prefix + "/control_point", value)) {
      ROS_WARN ("missing parameter: %s/control_point", prefix.c_str());
      for (int ii(0); ii < 3; ++ii) {
	control_point.push_back(0.0);
      }
    }
    else {
      if (3 != value.size()) {
	ROS_ERROR ("invalid %s/control_point", prefix.c_str());
	return 0;
      }
      for (int ii(0); ii < value.size(); ++ii) {
	control_point.push_back(value[ii]);
      }
    }
    
    std::vector<double> goal;
    if ( ! nn.getParam(prefix + "/task_goal", value)) {
      ROS_WARN ("missing parameter: %s/task_goal", prefix.c_str());
      for (int ii(0); ii < 3; ++ii) {
	goal.push_back(0.0);
      }
    }
      ...
      if (3 != value.size()) {
	ROS_ERROR ("invalid %s/task_goal", prefix.c_str());
	return 0;
      }
      for (int ii(0); ii < value.size(); ++ii) {
	goal.push_back(value[ii]);
      }
    }
    if ( ! nn.getParam(prefix + "/posture_goal", value)) {
      ROS_WARN ("missing parameter: %s/posture_goal", prefix.c_str());
      for (int ii(0); ii < jspace_model->getNDOF(); ++ii) {
	goal.push_back(0.0);
      }
    }
    else {
      if (jspace_model->getNDOF() != value.size()) {
	ROS_ERROR ("invalid %s/posture_goal", prefix.c_str());
	return 0;
      }
      for (int ii(0); ii < value.size(); ++ii) {
	goal.push_back(value[ii]);
    ...
    if ( ! nn.getParam(prefix + "/task_kp", value)) {
      ROS_WARN ("missing parameter: %s/task_kp", prefix.c_str());
      for (int ii(0); ii < 3; ++ii) {
	kp.push_back(100.0);
      }
    }
      ...
      if (3 != value.size()) {
	ROS_ERROR ("invalid %s/task_kp", prefix.c_str());
	return 0;
      }
      for (int ii(0); ii < value.size(); ++ii) {
	kp.push_back(value[ii]);
      }
    }
    if ( ! nn.getParam(prefix + "/posture_kp", value)) {
      ROS_WARN ("missing parameter: %s/posture_kp", prefix.c_str());
      for (int ii(0); ii < jspace_model->getNDOF(); ++ii) {
	kp.push_back(100.0);
      }
    }
      ...
      if (jspace_model->getNDOF() != value.size()) {
	ROS_ERROR ("invalid %s/posture_kp", prefix.c_str());
	return 0;
      }
      for (int ii(0); ii < value.size(); ++ii) {
	kp.push_back(value[ii]);
    ...
    if ( ! nn.getParam(prefix + "/task_kd", value)) {
      ROS_WARN ("missing parameter: %s/task_kd", prefix.c_str());
      for (int ii(0); ii < 3; ++ii) {
	kd.push_back(20.0);
      }
    }
      ...
      if (3 != value.size()) {
	ROS_ERROR ("invalid %s/task_kd", prefix.c_str());
	return 0;
      }
      for (int ii(0); ii < value.size(); ++ii) {
	kd.push_back(value[ii]);
      }
    }
    if ( ! nn.getParam(prefix + "/posture_kd", value)) {
      ROS_WARN ("missing parameter: %s/posture_kd", prefix.c_str());
      for (int ii(0); ii < jspace_model->getNDOF(); ++ii) {
	kd.push_back(20.0);
      }
    }
      ...
      if (jspace_model->getNDOF() != value.size()) {
	ROS_ERROR ("invalid %s/posture_kd", prefix.c_str());
	return 0;
      }
      for (int ii(0); ii < value.size(); ++ii) {
	kd.push_back(value[ii]);
    ...
    if ( ! nn.getParam(prefix + "/task_maxvel", value)) {
      ROS_WARN ("missing parameter: %s/task_maxvel", prefix.c_str());
      for (int ii(0); ii < 3; ++ii) {
	maxvel.push_back(0.6);
      }
    }
    else {
      if (3 != value.size()) {
	ROS_ERROR ("invalid %s/task_maxvel", prefix.c_str());
	return 0;
      }
      for (int ii(0); ii < value.size(); ++ii) {
	maxvel.push_back(value[ii]);
      }
    }
    if ( ! nn.getParam(prefix + "/posture_maxvel", value)) {
      ROS_WARN ("missing parameter: %s/posture_maxvel", prefix.c_str());
      for (int ii(0); ii < jspace_model->getNDOF(); ++ii) {
	maxvel.push_back(1.2);
      }
    }
    else {
      if (jspace_model->getNDOF() != value.size()) {
	ROS_ERROR ("invalid %s/posture_maxvel", prefix.c_str());
	return 0;
      }
      for (int ii(0); ii < value.size(); ++ii) {
	maxvel.push_back(value[ii]);
    ...
    if ( ! nn.getParam(prefix + "/task_maxacc", value)) {
      ROS_WARN ("missing parameter: %s/task_maxacc", prefix.c_str());
      for (int ii(0); ii < 3; ++ii) {
	maxacc.push_back(1.2);
      }
    }
    else {
      if (3 != value.size()) {
	ROS_ERROR ("invalid %s/task_maxacc", prefix.c_str());
	return 0;
      }
      for (int ii(0); ii < value.size(); ++ii) {
	maxacc.push_back(value[ii]);
      }
    }
    if ( ! nn.getParam(prefix + "/posture_maxacc", value)) {
      ROS_WARN ("missing parameter: %s/posture_maxacc", prefix.c_str());
      for (int ii(0); ii < jspace_model->getNDOF(); ++ii) {
	maxacc.push_back(2.4);
      }
    }
    else {
      if (jspace_model->getNDOF() != value.size()) {
	ROS_ERROR ("invalid %s/posture_maxacc", prefix.c_str());
	return 0;
      }
      for (int ii(0); ii < value.size(); ++ii) {
	maxacc.push_back(value[ii]);
    ...
    ctrl = new opspace::TaskPostureController();
    jspace::Status st(ctrl->setGoal(jspace::Vector::Map(&goal[0], goal.size())));
    if ( ! st) {
      throw runtime_error("opspace::TaskPostureController::setGoal() said " + st.errstr);
    }
    st = ctrl->setGains(jspace::Vector::Map(&kp[0], kp.size()),
			jspace::Vector::Map(&kd[0], kd.size()));
    if ( ! st) {
      throw runtime_error("opspace::TaskPostureController::setGains() said " + st.errstr);
    }
    st = ctrl->setCycleTime(dt_seconds);
    if ( ! st) {
      throw runtime_error("opspace::TaskPostureController::setCycleTime() said " + st.errstr);
    }
    st = ctrl->setEndEffector(end_effector);
    if ( ! st) {
      throw runtime_error("opspace::TaskPostureController::setEndEffector() said " + st.errstr);
    }
    st = ctrl->setControlPoint(jspace::Vector::Map(&control_point[0], control_point.size()));
    if ( ! st) {
      throw runtime_error("opspace::TaskPostureController::setControlPoint() said " + st.errstr);
    }
    st = ctrl->setMaxvel(jspace::Vector::Map(&maxvel[0], maxvel.size()));
    if ( ! st) {
      throw runtime_error("opspace::TaskPostureController::setMaxvel() said " + st.errstr);
    }
    st = ctrl->setMaxacc(jspace::Vector::Map(&maxacc[0], maxacc.size()));
    if ( ! st) {
      throw runtime_error("opspace::TaskPostureController::setMaxacc() said " + st.errstr);
    }
    
    ROS_INFO ("create_task_posture_controller(): SUCCESS, yeah!");
  }
  
  catch (XmlRpc::XmlRpcException const & ee) {
    ROS_ERROR ("XmlRpcException in create_task_posture_controller(): %s", ee.getMessage().c_str());
    delete ctrl;		// just in case...
    return 0;
  }
  
  catch (std::runtime_error const & ee) {
    ROS_ERROR ("Runtime error in create_task_posture_controller(): %s", ee.what());
    delete ctrl;		// just in case...
    return 0;
  }
